...
class JavaScriptGenerator extends BaseGenerator {
    ...
    public generate(templateContent: string): string {
        ...
        _this._addLine('import IDevicesViewModel = require(\'IDevicesViewModel\');');
        ...
        _this._addConstructor(template, interfaceName);
    ...
    private _addConstructor(template: CompiledViewTemplate, interfaceName: string) {
        var _this = this;

        _this._addLine('constructor(viewModel?: ' + interfaceName + ') {', 1);
        _this._addLine('super(viewModel);', 2);
        _this._addLine();
    ...
    private _addElementContent(element: HTMLElement, indent: number) {
        ...
        if (annotation) {
            ...
            if (annotation.html) {
                this._addLine('viewModel.' + annotation.html + ' +', indent);
            }
        }
    }
